...
public class SecurityConfig {
    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception { //función llamada securityFilterChain
        http.csrf(csrf -> csrf.disable()) // desactivamos una protección llamada CSRF
                .authorizeHttpRequests(authorize -> authorize
                        .requestMatchers("/api/register", "/api/login", "/productos", "/uploads/**", "/api/carreras").permitAll() // permite que cualquiera pueda acceder a estas rutas
    ...
    public WebSecurityCustomizer webSecurityCustomizer() {
        return (web) -> web.ignoring().requestMatchers("/api/register", "/api/login", "/productos", "/uploads/**", "/api/carreras");
